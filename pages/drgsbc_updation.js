...
function esOnFilterChange() {
  ...
  const s = matches[0]; // first match — unique combination in the common case
  ES.filtered = s;
  esFillHeaderStrip(s);
...
async function esFetchLineItems(sanctionId) {
  ...
  ES.actions = {}; ES.editValues = {}; ES.newItemSeq = 0;
...
function esRenderLineItems() {
  ...
  ES.lineItems.forEach((li, i) => {
    ...
    rows.push(`<tr style="${rowStyle}" data-li-id="${li.line_item_id}">
      ...
      <td class="pd-cell">
        <input class="pd-inp" type="date" value="${submitOn || ''}" style="width:115px;" data-es-estimate="e_submitted_on">
      </td>
      <td class="pd-cell">
        <input class="pd-inp" type="date" value="${vettedOn || ''}" min="${submitOn || ''}" style="width:115px;" data-es-estimate="e_vetted_on">
      </td>
      <td class="pd-cell">
  ...
  // New items (added via + NEW ITEM) live only in the DOM — re-append
  // their current outerHTML so they survive a re-render triggered by
  // an existing row's action changing.
  document.querySelectorAll('[data-es-new]').forEach(row => rows.push(row.outerHTML));
...
/* ================================================================
   ...
   ================================================================ */
let SI = {
  sanctionMap: {}, // under_power → [ { line_item_id, item_name, sanction_id, sanction_year, plan_head, total_cost } ]
  selectedLI: null,
  existingRows: [],
  newRows: [], // [ { seq, editId } ]
  editedIds: new Set(),
...
async function siLoadUnderPowers() {
  ...
  try {
    ...
    const cfg = getDbConfig();
    if (cfg?.nexus?.url && cfg?.nexus?.key) {
      data = await nxFetch('sanction_header?select=sanction_id,under_power,sanction_year,plan_head,sanctioned_amount,sanction_line_item(line_item_id,item_name)&order=sanction_year.desc');
...
async function siItemSelected() {
  ...
  if (!found) { siResetParentStrip(); return; }
  ...
  document.getElementById('si_disp_total').textContent = found.total_cost ? 'Rs. ' + Number(found.total_cost).toLocaleString('en-IN') : '—';
...
function siUpdateCostMatch() {
  const totalCost = parseFloat(SI.selectedLI?.total_cost) || 0;
...
async function siSubmitAll() {
  ...
  for (const nr of SI.newRows) {
    ...
    const payload = {
      ...
      // Fixed vs v16: defaults to this row's own consignee depot rather
      // than the always-undefined SI.selectedLI.processing_depot.
      processing_depot: cdepot,
      state: 'Active',
    };
...
async function pdRecalcFromTat(sid, stageName) {
  ...
  const stageRow = tat.find(t => t.status === stageName);
  if (!stageRow) return;
...
function pdBuildStageOptions(currentStatus, selectedStage) {
  ...
  const stages = isTerminal ? [] : statusStages;
  const allStages = [...new Set([...stages, ...universal])];
...
function pdRenderProcTable() {
  ...
  tbody.innerHTML = PD.filteredRows.map((r, i) => {
    ...
    return `<tr data-sid="${sid}">
      ...
      <td class="pd-cell edit pd-grp" data-grp="indent" data-col="indentno"><input class="pd-inp${dc('indent_number')}" type="text" data-sid="${sid}" data-field="indent_number" value="${v('indent_number')}" placeholder="—"></td>
      ...
      <td class="pd-cell edit pd-grp" data-grp="loapo" data-col="loaponumber"><input class="pd-inp${dc('loa_po_number')}" type="text" data-sid="${sid}" data-field="loa_po_number" value="${v('loa_po_number')}" placeholder="—"></td>
...
// Single delegated listener for the whole Procurement grid — replaces
// v16's per-cell inline onchange attributes with one handler keyed off
// data-field/data-sid, since the markup here has no inline JS.
...
async function pdSaveAll() {
  ...
  for (const [subItemId, changes] of Object.entries(PD.dirtyProc)) {
    ...
    if (errs.length) dateErrors.push(`Sub-item [${row.sub_item_name}]:\n  ` + errs.join('\n  '));
  }
    ...
    if (errTotal === 0) {
      ...
      for (const subItemId of savedSubIds) {
        try {
          ...
          const procDetail = {
            process_stage: row.process_stage, indent_date: row.indent_date, loa_po_date: row.loa_po_date,
            delivery_date: row.delivery_date, crn_date: row.crn_date,
          };
