...
const FIELD_GROUPS = [
  ...
  { group: 'LINE ITEM', fields: [
    ...
    // Fixed: was type:'text', which excluded it from CAT_FIELDS — the
    // sole source list for the Group By dropdown (see
    // populateGroupByAndSortSelects()) — so Line Item Name could never
    // be selected as a grouping field, even though it's functionally
    // used as a repeated category (same item name across many
    // sub-items) exactly like Under Power / Plan Head / Status already
    // are. Filter behavior is unaffected: 'cat' and 'text' share the
    // identical operator set and free-text value input (see
    // opsForType()) — only the Group By eligibility changes.
    { field:'item_name',             label:'Item Name',          type:'cat'  },
